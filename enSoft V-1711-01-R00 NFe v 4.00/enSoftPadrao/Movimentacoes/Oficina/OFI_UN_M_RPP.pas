...
type
  TOFI_FM_M_RPP = class(TPAD_FM_X_PAD)
...
var
  OFI_FM_M_RPP: TOFI_FM_M_RPP;

implementation

{$R *.dfm}

uses uDmGeral, PSQ_UN_X_ITE, PSQ_UN_X_FOR, OFI_UN_M_RPP_CAN, OFI_RN_M_ORV,
  OFI_UN_M_RPP_IMP, OFI_RN_M_RQV, FAT_RN_M_NFE{, OFI_UN_M_RQV};

procedure TOFI_FM_M_RPP.acAlterarExecute(Sender: TObject);
begin
...
procedure TOFI_FM_M_RPP.acCancelarExecute(Sender: TObject);
begin
   if Botoes(dso.DataSet, TAction(Sender).Tag,dmGeral.OFI_CD_M_ORV) then
     begin
       inherited;
...
procedure TOFI_FM_M_RPP.AbrirBuscas;
begin
...
procedure TOFI_FM_M_RPP.acExcluirExecute(Sender: TObject);
begin
  inherited;
  //Botoes(dso.DataSet, TAction(Sender).Tag,dmGeral.OFI_CD_M_ORV);
end;

procedure TOFI_FM_M_RPP.acGravarExecute(Sender: TObject);
var
  codigo: string;
begin
   if (dmGeral.CAD_CD_C_PAR_CTR.FieldByName('orv_id_tipo_mov_estoque').AsString = '') then
  ...
  if MessageDlg('Confirma a Gravação?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    begin
     ...
     dmGeral.OFI_CD_M_ORV.Data :=
          dmGeral.OFI_CD_M_ORV.DataRequest(VarArrayOf([0, codigo,'','2','4']));
     AbrirBuscas;
     btnImprimirEspelho.Enabled := true;
     btnAddItens.Enabled := true;
    end
...
procedure TOFI_FM_M_RPP.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  dmGeral.OFI_CD_M_ORV.Close;
  FreeAndNil(OFI_FM_M_RPP);
end;

procedure TOFI_FM_M_RPP.FormShow(Sender: TObject);
begin
  inherited;
  dmGeral.OFI_CD_M_ORV.Close;
...
procedure TOFI_FM_M_RPP.pmCancelarPopup(Sender: TObject);
begin
  inherited;
   if (dmGeral.OFI_CD_M_ORV_ITE.FieldByName('CANCELADA').AsBoolean = False) then
    begin
       OFI_FM_M_RPP.pmCancelar.Items[0].Enabled := True;
    end
  else
    begin
       OFI_FM_M_RPP.pmCancelar.Items[0].Enabled := False;
    end;
end;

procedure TOFI_FM_M_RPP.pnlDespesasExit(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_RPP.pnlItensExit(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_RPP.txtBuscaItemButtonClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_RPP.txtBuscaItemExit(Sender: TObject);
var
...
procedure TOFI_FM_M_RPP.ChamaMecanicoTerceiro;
begin
...
procedure TOFI_FM_M_RPP.txtBuscaItemKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
  if key = vk_f7 then
...
procedure TOFI_FM_M_RPP.txtDescricaoProblemaExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
       btnGrava.Focused  then
...
procedure TOFI_FM_M_RPP.txtLocalExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
       btnGrava.Focused  then
...
procedure TOFI_FM_M_RPP.txtPesquisaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TOFI_FM_M_RPP.txtQtdeExit(Sender: TObject);
var
...
procedure TOFI_FM_M_RPP.txtTerceiroButtonClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_RPP.txtValorExit(Sender: TObject);
var
  SMPrincipal : TSMClient;
  seq_item: string;
begin
...
procedure TOFI_FM_M_RPP.txtVlrUnitarioExit(Sender: TObject);
var
  SMPrincipal : TSMClient;
  seq_item: string;
begin
...
procedure TOFI_FM_M_RPP.dbGridCalcCellColors(Sender: TObject; Field: TField;
  State: TGridDrawState; Highlight: Boolean; AFont: TFont; ABrush: TBrush);
begin
  inherited;
...
procedure TOFI_FM_M_RPP.dgDespesasIButtonClick(Sender: TObject);
begin
  inherited;
  try
...
procedure TOFI_FM_M_RPP.dgDespesasMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TOFI_FM_M_RPP.dgItensCalcCellColors(Sender: TObject; Field: TField;
  State: TGridDrawState; Highlight: Boolean; AFont: TFont; ABrush: TBrush);
begin
  inherited;
...
procedure TOFI_FM_M_RPP.dgItensDblClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_RPP.dgItensIButtonClick(Sender: TObject);
begin
  inherited;
{ try
    dgItensIButton.Enabled := false;
...
procedure TOFI_FM_M_RPP.dgItensMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TOFI_FM_M_RPP.dpkDataExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
       btnGrava.Focused  then
...
procedure TOFI_FM_M_RPP.Atualizar_xBusca;
begin
  if xbusca_item = '' then
     begin
...
procedure TOFI_FM_M_RPP.btnAddDespesasClick(Sender: TObject);
begin
  inherited;
  pnlDespesas.Enabled  := True;
...
procedure TOFI_FM_M_RPP.btnAddItensClick(Sender: TObject);
begin
  inherited;
  pnlItens.Enabled  := True;
...
procedure TOFI_FM_M_RPP.btnAddItensKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
   if Key = vk_F2 then
...
procedure TOFI_FM_M_RPP.btnCancelarRegClick(Sender: TObject);
begin
  inherited;
  OFI_FM_M_RPP_CAN := TOFI_FM_M_RPP_CAN.Create(Self);
  OFI_FM_M_RPP_CAN.Show;
end;

procedure TOFI_FM_M_RPP.btnFiltroClick(Sender: TObject);
begin
  inherited;
  ...
  try
    ...
    if (cbbPesquisa.ItemIndex in [0,1] ) then
       begin
        ...
        dmGeral.OFI_CD_M_ORV.DataRequest(
                VarArrayOf([cbbPesquisa.ItemIndex,txtPesquisa.Text,dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').AsString,'2','4']));
       end;
    ...
    if (cbbPesquisa.ItemIndex in [2,3]) then
       begin
        ...
        dmGeral.OFI_CD_M_ORV.Data :=
        dmGeral.OFI_CD_M_ORV.DataRequest(
                VarArrayOf([cbbPesquisa.ItemIndex+1,txtPesquisa.Text,dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').AsString,'2','4']));
       end;
    ...
    if cbbPesquisa.ItemIndex = 4 then
       begin
        ...
        dmGeral.OFI_CD_M_ORV.Data :=
        dmGeral.OFI_CD_M_ORV.DataRequest(
                VarArrayOf([10,txtPesquisa.Text,dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').AsString,'2','4']));
       end;
    ...
    if cbbPesquisa.ItemIndex in [5,6] then
       begin
        ...
        dmGeral.OFI_CD_M_ORV.Data :=
        dmGeral.OFI_CD_M_ORV.DataRequest(
                VarArrayOf([cbbPesquisa.ItemIndex+8,txtPesquisa.Text,dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').AsString,'2','4']));
       end;
...
procedure TOFI_FM_M_RPP.btnImprimirEspelhoClick(Sender: TObject);
var
...
begin
  ...
  if not dmGeral.OFI_CD_M_ORV.IsEmpty then
      begin
        ...
        OFI_FM_M_RPP_IMP := TOFI_FM_M_RPP_IMP.Create(Self);
        OFI_FM_M_RPP_IMP.Show;
      end
   else
      begin
...
procedure TOFI_FM_M_RPP.cbbMecanicoEnter(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_RPP.cbbMecanicoExit(Sender: TObject);
var
  seq_item:string;
begin
  inherited;
...
procedure TOFI_FM_M_RPP.cbbPesquisaChange(Sender: TObject);
begin
  inherited;
  txtPesquisa.Text := '';
...
procedure TOFI_FM_M_RPP.cbbTerceiroEnter(Sender: TObject);
begin
  inherited;
  dmGeral.BusFornecedor(1,'%');
...
procedure TOFI_FM_M_RPP.cbbTerceiroExit(Sender: TObject);
var
  seq_item:string;
begin
  inherited;
...
procedure TOFI_FM_M_RPP.cbbTipoEnter(Sender: TObject);
begin
  inherited;
  dmGeral.BUS_PR_X_ORV_DES_TIP;
  cbbTipo.DropDown;
...
procedure TOFI_FM_M_RPP.cbbTipoExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused then
