...
uses
  ...
  dxSkinWhiteprint, dxSkinXmas2008Blue, Vcl.ComCtrls, enFunc, Datasnap.DBClient,
  dxSkinMetropolis, dxSkinMetropolisDark, dxSkinOffice2013DarkGray,
  dxSkinOffice2013LightGray, dxSkinOffice2013White;

type
  TOFI_FM_M_FEV = class(TPAD_FM_X_PAD)
    gbIOR: TGroupBox;
    lblNOrdem: TLabel;
    lblCliente: TLabel;
    txtNOrdem: TwwDBEdit;
  ...
  private
    ...
    procedure DesabilitarCamposOSGarantia(ativar:Boolean);
    var
      xCondPag,id_perfil_cli:string;
      xCred_disp, xCred_Util,xCred_Util_pec,xCred_Util_ser:currency;
      xId_titulo_pec,xId_titulo_ser:integer;
      mmCabProMec,mmCabSerMec,mmSerMec,mmSerItem,mmProMec,mmProItem:TfrxMemoView;
    public
    { Public declarations }
  end;
var
  OFI_FM_M_FEV: TOFI_FM_M_FEV;
...
uses uDmGeral, PSQ_UN_X_FPG, OFI_RN_M_FEV, OFI_RN_M_ORV, enConstantes,
  OFI_UN_M_ORV_CAN, OFI_UN_M_FEV_OPC;

procedure TOFI_FM_M_FEV.acAdicionaExecute(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEV.acAlterarExecute(Sender: TObject);
VAR
  msg:String;
begin
  msg := '';
  ...
  if (dmGeral.CAD_CD_C_PAR_CTR.FieldByName('ORV_ID_FORMA_PAG_RCR').AsString = '') then
    begin
      msg := msg +'Forma de pagamento para nota de credito deve ser preenchido na tela de parâmetros.'+#13+#10;
    end;
  ...
  if Botoes(dso.DataSet, TAction(Sender).Tag,dmGeral.OFI_CD_M_ORV) then
    begin
     ...
     if (dmGeral.OFI_CD_M_ORV.FieldByName('INT_TIPO_OS').AsInteger = 0) then
       begin
        OFI_FM_M_FEV_OPC := TOFI_FM_M_FEV_OPC.Create(Self);
        OFI_FM_M_FEV_OPC.ShowModal;
        FreeAndNil(OFI_FM_M_FEV_OPC);
       end
     else if (dmGeral.OFI_CD_M_ORV.FieldByName('INT_TIPO_OS').AsInteger = 1) then
       begin
    ...
    { if ((dmGeral.CAD_CD_C_PAR_CTR.FieldBYName('calcular_iss_retido').AsBoolean = true) and
     ...
     if (dmGeral.BUS_CD_C_FUN.FieldByName('SEG_CANCELAR_OS').AsBoolean=true) then
       btmCancelamento.Enabled := false;
     DesabilitarCamposOSGarantia(false);
...
procedure TOFI_FM_M_FEV.DesabilitarCamposOSGarantia(ativar:Boolean);
begin
  if (ativar = false) then
    begin
     if (dmGeral.OFI_CD_M_ORV.FieldByName('INT_TIPO_OS').AsInteger = 0) then
        begin
          cbbCondPag.Enabled := true;
          txtCredServico.Enabled := true;
          txtpecperdesc.Enabled := true;
          txtPecDesc.Enabled := true;
          txtserperdesc.Enabled := true;
          txtServDesc.Enabled := true;
          txtterperdesc.Enabled := true;
          txtTerDesc.Enabled := true;
          gbTit.Enabled := true;
          gbPG.Enabled := true;
        end
     else if (dmGeral.OFI_CD_M_ORV.FieldByName('INT_TIPO_OS').AsInteger = 1) then
        begin
          ...
          txtCredServico.Enabled := false;
          txtpecperdesc.Enabled := false;
          txtPecDesc.Enabled := false;
          txtserperdesc.Enabled := false;
          txtServDesc.Enabled := false;
          txtterperdesc.Enabled := false;
          txtTerDesc.Enabled := false;
          gbTit.Enabled := false;
          gbPG.Enabled := false;
          txtCredGer.SetFocus;
        end;
...
procedure TOFI_FM_M_FEV.acCancelarExecute(Sender: TObject);
begin
  inherited;
  if Botoes(dso.DataSet, TAction(Sender).Tag,dmGeral.OFI_CD_M_ORV) then
     begin
       ...
       HabDesabCampos;
       DesabilitarCamposOSGarantia(true);
     end
  else
     begin
      // cbbCondPag.SetFocus;
...
procedure TOFI_FM_M_FEV.HabDesabCampos;
begin
...
procedure TOFI_FM_M_FEV.pnlTitExit(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEV.pnlTitSerExit(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEV.acExcluirExecute(Sender: TObject);
begin
  inherited;
//   Botoes(dso.DataSet, TAction(Sender).Tag,dmGeral.OFI_CD_M_ORV);
end;

procedure TOFI_FM_M_FEV.acGravarExecute(Sender: TObject);
var
  codigo: string;
  ...
  if MessageDlg('Confirma a Gravação?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
     begin
         ...
         dmGeral.OFI_CD_M_ORV.Data :=
                dmGeral.OFI_CD_M_ORV.DataRequest(VarArrayOf([0, codigo,dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').AsString,'3','1']));
         AbrirBusca;
         txtCredDisp.Text := '';
         xcred_disp := 0;
         btnImprimirEspelho.Enabled := true;
...
procedure TOFI_FM_M_FEV.AbrirBusca;
begin
  dmGeral.BusFormaPgtos(1,'%');
  dmGeral.BusCondPgto(1,'%');
end;

procedure TOFI_FM_M_FEV.btmCancelamentoClick(Sender: TObject);
var
  codFiscal:String;
begin
  inherited;
         ...
         begin
            if (dmGeral.OFI_CD_M_ORV.FieldByName('INT_TIPO_OS').AsInteger = 0) then
              begin
                 ...
                 dmGeral.BUS_CD_M_NFE.DataRequest(
                    VarArrayOf([13,dmGeral.OFI_CD_M_ORV.FieldByName('ID_ORDEM').AsInteger]));
              end;
            ...
            if (dmGeral.OFI_CD_M_ORV.FieldByName('INT_TIPO_OS').AsInteger = 1) then
              begin
                ...
                dmGeral.BUS_CD_M_NFE.Data :=
                dmGeral.BUS_CD_M_NFE.DataRequest(
                  VarArrayOf([116,dmGeral.OFI_CD_M_ORV.FieldByName('ID_EMPRESA').AsString,
                                  dmGeral.OFI_CD_M_ORV.FieldByName('ID_ORDEM').AsString]));
              end;
            ...
            if codFiscal <> '' then
              begin
                ShowMessage('Ordem de serviço não pode ser cancelado, pois há seguintes notas pendentes que devem ser antes canceladas ou inutilizadas:'+#13+#10+CodFiscal);
                exit;
              end;
           ...
           if MessageDlg('Deseja realmente cancelar ordem de serviço?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
             begin
              ...
              OFI_FM_M_ORV_CAN.xFormCall := 'OFI_FM_M_FEV';
...
procedure TOFI_FM_M_FEV.BtnaddTitPecSerClick(Sender: TObject);
begin
  inherited;
  pnlTit.Enabled  := True;
...
procedure TOFI_FM_M_FEV.btnAddTitSerClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEV.btnFiltroClick(Sender: TObject);
var
  xPesquisa:String;
begin
  inherited;
  ...
  try
    ...
    dmGeral.OFI_CD_M_ORV.DataRequest(
            VarArrayOf([cbbPesquisa.ItemIndex, xPesquisa,dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').AsString,'3','1']));
  finally
...
procedure TOFI_FM_M_FEV.btnImprimirEspelhoClick(Sender: TObject);
var
      ...
      begin
        ...
        if FileExists(PathImg) then
          ...
          if (dmGeral.CAD_CD_C_PAR_CTR.FieldByName('UTILIZA_MECANICO').AsBoolean = false) then
            begin
                TfrxMemoView (OFI_FR_M_FEV.FindComponent('mmCabProMec')).Visible := false;
                mmProMec := TfrxMemoView (OFI_FR_M_FEV.FindComponent('mmProMec'));
                mmProMec.Visible := false;
                mmProItem := TfrxMemoView (OFI_FR_M_FEV.FindComponent('mmProItem'));
                mmProItem.Width :=   mmProItem.Width +  mmProMec.Width;

                TfrxMemoView (OFI_FR_M_FEV.FindComponent('mmCabSerMec')).Visible := false;
                mmSerMec := TfrxMemoView (OFI_FR_M_FEV.FindComponent('mmSerMec'));
                mmSerMec.Visible := false;
                mmSerItem := TfrxMemoView (OFI_FR_M_FEV.FindComponent('mmSerItem'));
                mmSerItem.Width :=   mmSerItem.Width +  mmSerMec.Width;
            end;
        ...
        dmGeral.BusCodigoRevListMestre(true,false,OFI_FM_M_FEV.Name,xCodLme,xRevLme,nil);
...
procedure TOFI_FM_M_FEV.btnReaberturaClick(Sender: TObject);
VAR
  codFiscal:String;
begin
  inherited;
     ...
     begin
       ...
       if (dmGeral.OFI_CD_M_ORV.FieldByName('STATUS').AsInteger = 2) then
         begin
            ...
            if codFiscal <> '' then
              begin
                ShowMessage('Ordem de serviço não pode ser reaberto, pois há seguintes notas pendentes que devem ser antes canceladas ou inutilizadas:'+#13+#10+CodFiscal);
                exit;
              end;
            dmgeral.OFI_CD_M_ORV.edit;
...
procedure TOFI_FM_M_FEV.cbbCondPagEnter(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEV.cbbCondPagExit(Sender: TObject);
var
 id_categoria,tipo_pagamento:Integer;
begin
  inherited;
  ...
  tipo_pagamento := dmGeral.BUS_CD_C_CPG.FieldByName('TIPO_PAGAMENTO').AsInteger;
  if (xCondPag <> dmGeral.OFI_CD_M_ORV.FieldByName('ID_CONDICAO_PAG').AsString) then
     begin
       while dmGeral.OFI_CD_M_ORV_TIT.RecordCount >0 do
         begin
                  ...
                  begin
                    if (dmGeral.OFI_CD_M_ORV_ITE.FieldByName('CANCELADA').AsBoolean = False) then
                      begin
                       ...
                       dmGeral.CalcPrecoItem(1,id_perfil_cli,dmGeral.OFI_CD_M_ORV.FieldByName('ID_CONDICAO_PAG').Text,
...
procedure TOFI_FM_M_FEV.cbbFormaPagSerEnter(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEV.cbbFormaPagSerExit(Sender: TObject);
begin
  inherited;
   if btnCancelar.Focused or
     btnGrava.Focused then
...
procedure TOFI_FM_M_FEV.cbbFormPagPecSerEnter(Sender: TObject);
begin
  inherited;
   dmGeral.BUS_CD_C_CPG.Close;
  dmGeral.BUS_CD_C_CPG.Data :=
...
procedure TOFI_FM_M_FEV.cbbFormPagPecSerExit(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEV.cbbPesquisaChange(Sender: TObject);
begin
  inherited;
  txtPesquisa.Text := '';
...
procedure TOFI_FM_M_FEV.dbGridCalcCellColors(Sender: TObject; Field: TField;
  State: TGridDrawState; Highlight: Boolean; AFont: TFont; ABrush: TBrush);
begin
  inherited;
...
procedure TOFI_FM_M_FEV.dgTitPecSerIButtonClick(Sender: TObject);
var
  total_cred:currency;
begin
  inherited;
...
procedure TOFI_FM_M_FEV.dgTitPecSerMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TOFI_FM_M_FEV.dgTitPecSerRowChanged(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEV.dgTitSerIButtonClick(Sender: TObject);
var
  total_cred:currency;
begin
  inherited;
...
procedure TOFI_FM_M_FEV.dgTitSerMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TOFI_FM_M_FEV.dgTitSerRowChanged(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEV.ExibirFormasPgto;
begin
  dmGeral.BUS_CD_C_CPG.Close;
  dmGeral.BUS_CD_C_CPG.Data :=
          dmGeral.BUS_CD_C_CPG.DataRequest(VarArrayOf([0,dmGeral.OFI_CD_M_ORV.FieldByName('ID_CONDICAO_PAG').AsString]));
...
procedure TOFI_FM_M_FEV.dpkVencimentoPecSerExit(Sender: TObject);
begin
  inherited;
   if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEV.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  dmGeral.OFI_CD_M_ORV.Close;
  FreeAndNil(OFI_FM_M_FEV);
end;

procedure TOFI_FM_M_FEV.FormShow(Sender: TObject);
begin
  inherited;
  dmGeral.OFI_CD_M_ORV.Close;
  dmGeral.OFI_CD_M_ORV.Data := dmGeral.OFI_CD_M_ORV.DataRequest(VarArrayOf([0, '','','3','1']));
  dmGeral.OFI_CD_M_ORV.Open;


  dmGeral.BusFuncionario(0,IntToStr(xFuncionario));
  if (not (dmGeral.BUS_CD_C_FUN.FieldByName('SEG_CANCELAR_OS').AsBoolean=true)) then
    btmCancelamento.Enabled := false;
...
procedure TOFI_FM_M_FEV.txtAgenciaPecSerExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEV.txtAgenciaSerExit(Sender: TObject);
begin
  inherited;
   if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEV.txtBancoPecSerExit(Sender: TObject);
begin
  inherited;
   if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEV.txtBancoSerExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEV.txtChequePecSerExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEV.txtChequeSerExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEV.txtContaPecSerExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEV.txtContaSerExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEV.txtCredGerExit(Sender: TObject);
var
  total_cred:currency;
begin
  inherited;
...
procedure TOFI_FM_M_FEV.txtCredPecasExit(Sender: TObject);
var
  total_cred:currency;
begin
  inherited;
...
procedure TOFI_FM_M_FEV.txtCredServicoExit(Sender: TObject);
var
  total_cred:currency;
begin
  inherited;
...
procedure TOFI_FM_M_FEV.txtDiasPecSerExit(Sender: TObject);
begin
  inherited;
   if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEV.txtDiasSerExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEV.txtEmitentePecSerExit(Sender: TObject);
 var
  id_titulo: string;
  cheValido:Boolean;
begin
...
procedure TOFI_FM_M_FEV.txtEmitenteSerExit(Sender: TObject);
 var
  id_titulo: string;
  cheValido:Boolean;
begin
...
procedure TOFI_FM_M_FEV.txtFpgEntradaButtonClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEV.txtFpgEntradaEnter(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEV.txtFpgEntradaExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEV.txtGerFpgButtonClick(Sender: TObject);
begin
  inherited;
  PSQ_FM_X_FPG := TPSQ_FM_X_FPG.Create(Self);
...
procedure TOFI_FM_M_FEV.txtGerFpgExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEV.txtPesquisaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TOFI_FM_M_FEV.txtQtdeParcExit(Sender: TObject);
var
  valor:Currency;
begin
  inherited;
...
procedure TOFI_FM_M_FEV.txtValorPecSerExit(Sender: TObject);
var
  id_titulo: string;
begin
  inherited;
...
procedure TOFI_FM_M_FEV.txtValorSerExit(Sender: TObject);
var
  id_titulo: string;
begin
  inherited;
...
procedure TOFI_FM_M_FEV.dpkVencimentoSerExit(Sender: TObject);
begin
  inherited;
   if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEV.txtVlrEntradaExit(Sender: TObject);
begin
  inherited;
   if btnCancelar.Focused or
     btnGrava.Focused  then
        ...
        if dmGeral.OFI_CD_M_ORV.FieldByName('orv_gera_fin_separado').AsBoolean then
           ...
           begin
             if ((dmGeral.OFI_CD_M_ORV.FieldByName('VLR_PROD_LIQUIDO').AsCurrency +
                 ...
                 begin
                   ShowMessage('O valor da soma do (valor líquido dos produtos + despesas + serviços) é zero.');
                   txtFpgEntrada.text := '';
                   txtVlrEntrada.Text := '0';
                   exit;
                 end;
...
procedure TOFI_FM_M_FEV.txtVlrParcelaExit(Sender: TObject);
var
  valor:Currency;
begin
  inherited;
