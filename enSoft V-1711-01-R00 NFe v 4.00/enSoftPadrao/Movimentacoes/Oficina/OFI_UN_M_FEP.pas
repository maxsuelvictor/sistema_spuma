...
type
  TOFI_FM_M_FEP = class(TPAD_FM_X_PAD)
    gbIOR: TGroupBox;
    lblNOrdem: TLabel;
    lblCliente: TLabel;
    txtNOrdem: TwwDBEdit;
...
var
  OFI_FM_M_FEP: TOFI_FM_M_FEP;

implementation

{$R *.dfm}

uses uDmGeral, PSQ_UN_X_FPG, OFI_RN_M_FEV, OFI_RN_M_ORV, enConstantes, OFI_UN_M_FEV_OPC,
    OFI_UN_M_ORP_CAN
  {OFI_UN_M_ORV_CAN, OFI_UN_M_FEV_OPC, OFI_UN_M_ORQ_CAN};

procedure TOFI_FM_M_FEP.acAdicionaExecute(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEP.acAlterarExecute(Sender: TObject);
begin
  ...
  if (dmGeral.OFI_CD_M_ORV.FieldByName('STATUS').AsInteger in [2,3,4,5]) then
     begin
  ...
  if Botoes(dso.DataSet, TAction(Sender).Tag,dmGeral.OFI_CD_M_ORV) then
    begin
     ...
     {if (dmGeral.CAD_CD_C_PAR_MOD.FieldByName('TOR').AsBoolean = false) then
       begin
...
procedure TOFI_FM_M_FEP.acCancelarExecute(Sender: TObject);
begin
  inherited;
  if Botoes(dso.DataSet, TAction(Sender).Tag,dmGeral.OFI_CD_M_ORV) then
     begin
...
procedure TOFI_FM_M_FEP.HabDesabCampos;
begin
...
procedure  TOFI_FM_M_FEP.HabilDesabCamposCheque;
begin
   dmGeral.BusFormaPgtos(0,dmGeral.OFI_CD_M_ORV_TIT.FieldByName('ID_FORMA_PAG').Text);
...
procedure  TOFI_FM_M_FEP.HabilDesabCamposChequeServico;
begin
...
procedure TOFI_FM_M_FEP.lblTituloClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEP.pnlTitExit(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEP.pnlTitSerExit(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEP.acExcluirExecute(Sender: TObject);
begin
  inherited;
//   Botoes(dso.DataSet, TAction(Sender).Tag,dmGeral.OFI_CD_M_ORV);
end;

procedure TOFI_FM_M_FEP.acGravarExecute(Sender: TObject);
var
  codigo: string;
  ...
  if MessageDlg('Confirma a Gravação?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
     begin
         ...
         dmGeral.OFI_CD_M_ORV.Data :=
                dmGeral.OFI_CD_M_ORV.DataRequest(VarArrayOf([0, codigo,
                                                  dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').AsString,'3','4']));
         AbrirBusca;
         txtCredDisp.Text := '';
         xcred_disp := 0;
         btnImprimirEspelho.Enabled := true;
...
procedure TOFI_FM_M_FEP.AbrirBusca;
begin
  dmGeral.BusFormaPgtos(1,'%');
  dmGeral.BusCondPgto(1,'%');
end;

procedure TOFI_FM_M_FEP.btmCancelamentoClick(Sender: TObject);
var
  codFiscal:String;
begin
  inherited;
       ...
       if (dmGeral.OFI_CD_M_ORV.FieldByName('STATUS').AsInteger in [0,1,2]) then
         ...
         if (dmGeral.OFI_CD_M_ORV.FieldByName('INT_TIPO_OS').AsInteger = 0) then
           ...
           if MessageDlg('Deseja realmente cancelar ordem de serviço?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
             begin
              OFI_FM_M_ORP_CAN := TOFI_FM_M_ORP_CAN.Create(Self);
              OFI_FM_M_ORP_CAN.Show;
...
procedure TOFI_FM_M_FEP.BtnaddTitPecSerClick(Sender: TObject);
begin
  inherited;
  pnlTit.Enabled  := True;
...
procedure TOFI_FM_M_FEP.btnAddTitSerClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEP.btnFiltroClick(Sender: TObject);
begin
  inherited;
  ...
  try
    ...
    if cbbPesquisa.ItemIndex in [0,1] then
       begin
          ...
          dmGeral.OFI_CD_M_ORV.DataRequest(
                  VarArrayOf([cbbPesquisa.ItemIndex, txtPesquisa.Text,dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').AsString,'3','4']));
       end;
    ...
    if cbbPesquisa.ItemIndex in [2,3] then
       begin
          ...
          dmGeral.OFI_CD_M_ORV.Data :=
          dmGeral.OFI_CD_M_ORV.DataRequest(
                  VarArrayOf([cbbPesquisa.ItemIndex+1, txtPesquisa.Text,dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').AsString,'3','4']));
       end;
    ...
    if cbbPesquisa.ItemIndex in [4,5] then
       begin
          ...
          dmGeral.OFI_CD_M_ORV.Data :=
          dmGeral.OFI_CD_M_ORV.DataRequest(
                  VarArrayOf([cbbPesquisa.ItemIndex+5, txtPesquisa.Text,dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').AsString,'3','4']));
       end;
...
procedure TOFI_FM_M_FEP.btnImprimirEspelhoClick(Sender: TObject);
var
...
begin
  ...
  if not dmGeral.OFI_CD_M_ORV.IsEmpty then
      begin
        ...
        dmGeral.BusCodigoRevListMestre(true,false,OFI_FM_M_FEP.Name,xCodLme,xRevLme,nil);
...
procedure TOFI_FM_M_FEP.btnReaberturaClick(Sender: TObject);
VAR
  codFiscal:String;
begin
  inherited;
...
procedure TOFI_FM_M_FEP.cbbCondPagEnter(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEP.cbbCondPagExit(Sender: TObject);
begin
  inherited;
  ...
  if (xCondPag <> dmGeral.OFI_CD_M_ORV.FieldByName('ID_CONDICAO_PAG').AsString) then
     begin
       ...
       if (dmGeral.OFI_CD_M_ORV_ITE.RecordCount > 0) then
         begin
           if MessageDlg('Se alterar condição de pagamento irá alterar os valores unitários dos itens. Deseja continuar?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
              begin
...
procedure TOFI_FM_M_FEP.cbbFormaPagSerEnter(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEP.cbbFormaPagSerExit(Sender: TObject);
begin
  inherited;
   if btnCancelar.Focused or
     btnGrava.Focused then
...
procedure TOFI_FM_M_FEP.cbbFormPagPecSerEnter(Sender: TObject);
begin
  inherited;
   dmGeral.BUS_CD_C_CPG.Close;
  dmGeral.BUS_CD_C_CPG.Data :=
...
procedure TOFI_FM_M_FEP.cbbFormPagPecSerExit(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEP.cbbPesquisaChange(Sender: TObject);
begin
  inherited;
  txtPesquisa.Text := '';
...
procedure TOFI_FM_M_FEP.dbGridCalcCellColors(Sender: TObject; Field: TField;
  State: TGridDrawState; Highlight: Boolean; AFont: TFont; ABrush: TBrush);
begin
  inherited;
...
procedure TOFI_FM_M_FEP.dgTitPecSerIButtonClick(Sender: TObject);
var
  total_cred:currency;
begin
  inherited;
...
procedure TOFI_FM_M_FEP.dgTitPecSerMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TOFI_FM_M_FEP.dgTitPecSerRowChanged(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEP.dgTitSerIButtonClick(Sender: TObject);
var
  total_cred:currency;
begin
  inherited;
...
procedure TOFI_FM_M_FEP.dgTitSerMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TOFI_FM_M_FEP.dgTitSerRowChanged(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEP.ExibirFormasPgto;
begin
  dmGeral.BUS_CD_C_CPG.Close;
  dmGeral.BUS_CD_C_CPG.Data :=
          dmGeral.BUS_CD_C_CPG.DataRequest(VarArrayOf([0,dmGeral.OFI_CD_M_ORV.FieldByName('ID_CONDICAO_PAG').AsString]));
...
procedure TOFI_FM_M_FEP.dpkVencimentoPecSerExit(Sender: TObject);
begin
  inherited;
   if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEP.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  dmGeral.OFI_CD_M_ORV.Close;
  FreeAndNil(OFI_FM_M_FEP);
end;

procedure TOFI_FM_M_FEP.FormShow(Sender: TObject);
begin
  inherited;

  dmGeral.OFI_CD_M_ORV.Close;
  dmGeral.OFI_CD_M_ORV.Data := dmGeral.OFI_CD_M_ORV.DataRequest(VarArrayOf([0, '','','3']));
  dmGeral.OFI_CD_M_ORV.Open;


  if (dmGeral.CAD_CD_C_PAR_MOD.FieldByName('TOR').AsBoolean=true) then
    begin
     tsPecSer.TabVisible :=false;
     cbbTitPecaServico.ItemIndex := 1;
     cbbTitPecaServico.Enabled := false;
     lblCredPecas.Visible := false;
     txtCredPecas.Visible := false;
    end;
...
procedure TOFI_FM_M_FEP.txtAgenciaPecSerExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEP.txtAgenciaSerExit(Sender: TObject);
begin
  inherited;
   if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEP.txtBancoPecSerExit(Sender: TObject);
begin
  inherited;
   if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEP.txtBancoSerExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEP.txtChequePecSerExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEP.txtChequeSerExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEP.txtContaPecSerExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEP.txtContaSerExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEP.txtCredGerExit(Sender: TObject);
var
  total_cred:currency;
begin
  inherited;
...
procedure TOFI_FM_M_FEP.txtCredPecasExit(Sender: TObject);
var
  total_cred:currency;
begin
  inherited;
...
procedure TOFI_FM_M_FEP.txtCredServicoExit(Sender: TObject);
var
  total_cred:currency;
begin
  inherited;
...
procedure TOFI_FM_M_FEP.txtDiasPecSerExit(Sender: TObject);
begin
  inherited;
   if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEP.txtDiasSerExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEP.txtEmitentePecSerExit(Sender: TObject);
 var
  id_titulo: string;
  cheValido:Boolean;
begin
...
procedure TOFI_FM_M_FEP.txtEmitenteSerExit(Sender: TObject);
 var
  id_titulo: string;
  cheValido:Boolean;
begin
...
procedure TOFI_FM_M_FEP.txtFpgEntradaButtonClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEP.txtFpgEntradaEnter(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_FEP.txtFpgEntradaExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEP.txtGerFpgButtonClick(Sender: TObject);
begin
  inherited;
  PSQ_FM_X_FPG := TPSQ_FM_X_FPG.Create(Self);
...
procedure TOFI_FM_M_FEP.txtGerFpgExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEP.txtPesquisaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TOFI_FM_M_FEP.txtQtdeParcExit(Sender: TObject);
var
  valor:Currency;
begin
  inherited;
...
procedure TOFI_FM_M_FEP.txtValorPecSerExit(Sender: TObject);
var
  id_titulo: string;
begin
  inherited;
...
procedure TOFI_FM_M_FEP.txtValorSerExit(Sender: TObject);
var
  id_titulo: string;
begin
  inherited;
...
procedure TOFI_FM_M_FEP.dpkVencimentoSerExit(Sender: TObject);
begin
  inherited;
   if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEP.txtVlrEntradaExit(Sender: TObject);
begin
  inherited;
   if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_FEP.txtVlrParcelaExit(Sender: TObject);
var
  valor:Currency;
begin
  inherited;
  ...
  if ((dmGeral.OFI_CD_M_ORV.FieldByName('SEPARA_PROD_SERV').AsBoolean  = True) and
     ...
     begin
      { if (valor = 0) then
       ...
       txtQtdeParc.text := '0';
       txtVlrParcela.Text := '0';
       pcTitulos.TabIndex := tsServicos.TabIndex;
