...
type
  TOFI_FM_M_RQV = class(TPAD_FM_X_PAD)
    ...
    OFI_FR_M_RQV_A4: TfrxReport;
    OFI_FR_M_RQV_8MM: TfrxReport;
   ...
   // procedure txtTerceiroExit(Sender: TObject);
    ...
    procedure btnAddItensKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure OFI_DB_M_RQV_ITECheckEOF(Sender: TObject; var Eof: Boolean);
...
var
  OFI_FM_M_RQV: TOFI_FM_M_RQV;

implementation

{$R *.dfm}

uses uDmGeral, PSQ_UN_X_ITE, PSQ_UN_X_FOR, OFI_UN_M_RQV_CAN, OFI_RN_M_ORV,
  OFI_UN_M_RQV_IMP, OFI_RN_M_RQV, FAT_RN_M_NFE;

procedure TOFI_FM_M_RQV.acAlterarExecute(Sender: TObject);
begin

  if (dmgeral.OFI_CD_M_ORV.FieldByName('INT_TIPO_OS').AsInteger = 1) then
    begin
      ShowMessage('Ordem de serviço é tipo garantia.');
      abort;
    end;
...
procedure TOFI_FM_M_RQV.acCancelarExecute(Sender: TObject);
begin
   if Botoes(dso.DataSet, TAction(Sender).Tag,dmGeral.OFI_CD_M_ORV) then
     begin
       inherited;
...
procedure TOFI_FM_M_RQV.AbrirBuscas;
begin
...
procedure TOFI_FM_M_RQV.acExcluirExecute(Sender: TObject);
begin
  inherited;
  //Botoes(dso.DataSet, TAction(Sender).Tag,dmGeral.OFI_CD_M_ORV);
end;

procedure TOFI_FM_M_RQV.acGravarExecute(Sender: TObject);
var
  codigo: string;
begin
   if (dmGeral.CAD_CD_C_PAR_CTR.FieldByName('orv_id_tipo_mov_estoque').AsString = '') then
       ...
       if (dmGeral.OFI_CD_M_ORV_ITE.FieldByName('CANCELADA').AsBoolean = False) then
          begin
           if ((dmGeral.OFI_CD_M_ORV_ITE.FieldByName('INT_TIPO_ITEM').AsString = '09') and
              ...
              ((dmgeral.CAD_CD_C_PAR_CTR.FieldByName('utiliza_mecanico').AsBoolean=true) and
  ...
  if MessageDlg('Confirma a Gravação?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
   ...
   //  OFI_RN_M_RQV.OfiCalculaVlrTotaisRqv;
     ...
     dmGeral.OFI_CD_M_ORV.Data :=
          dmGeral.OFI_CD_M_ORV.DataRequest(VarArrayOf([0, codigo,'','2','1']));
     AbrirBuscas;
     btnImprimirEspelho.Enabled := true;
    end
   else
...
procedure TOFI_FM_M_RQV.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  dmGeral.BUS_CD_C_VEI.Close;
  dmGeral.BUS_CD_C_CLI.Close;
  dmGeral.OFI_CD_M_ORV.Close;
  FreeAndNil(OFI_FM_M_RQV);
end;

procedure TOFI_FM_M_RQV.FormShow(Sender: TObject);
begin
  inherited;
  dmGeral.OFI_CD_M_ORV.Close;
  dmGeral.OFI_CD_M_ORV.Data := dmGeral.OFI_CD_M_ORV.DataRequest(VarArrayOf([0, '','','2','1']));
...
procedure TOFI_FM_M_RQV.OFI_DB_M_RQV_ITECheckEOF(Sender: TObject;
  var Eof: Boolean);
begin
  inherited;
  OFI_FR_M_RQV2.Variables['Mecanico'] := QuotedStr(dmGeral.BUS_CD_C_FUN_MECnome.Text);
  OFI_FR_M_RQV.Variables['Mecanico'] := QuotedStr(dmGeral.BUS_CD_C_FUN_MECnome.Text);
  OFI_FR_M_RQV_8MM.Variables['Mecanico'] := QuotedStr(dmGeral.BUS_CD_C_FUN_MECnome.Text);
  OFI_FR_M_RQV_A4.Variables['Mecanico'] := QuotedStr(dmGeral.BUS_CD_C_FUN_MECnome.Text);
end;

procedure TOFI_FM_M_RQV.pmCancelarPopup(Sender: TObject);
begin
  inherited;
   if (dmGeral.OFI_CD_M_ORV_ITE.FieldByName('CANCELADA').AsBoolean = False) then
    begin
       OFI_FM_M_RQV.pmCancelar.Items[0].Enabled := True;
    end
  else
    begin
       OFI_FM_M_RQV.pmCancelar.Items[0].Enabled := False;
    end;
end;

procedure TOFI_FM_M_RQV.pnlDespesasExit(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_RQV.pnlItensExit(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_RQV.txtBuscaItemButtonClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_RQV.txtBuscaItemExit(Sender: TObject);
var
...
procedure TOFI_FM_M_RQV.ChamaMecanicoTerceiro;
begin

  if (dmGeral.CAD_CD_C_PAR_CTR.FieldByName('informar_mec_por_item_os').AsBoolean=true) and
     (dmGeral.CAD_CD_C_PAR_CTR.FieldByName('UTILIZA_MECANICO').AsBoolean = true) and  ((dmGeral.OFI_CD_M_ORV_ITE.FieldByName('INT_TIPO_ITEM').AsString <> '09') or
...
procedure TOFI_FM_M_RQV.txtBuscaItemKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
  if key = vk_f7 then
...
procedure TOFI_FM_M_RQV.txtDescricaoProblemaExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
       btnGrava.Focused  then
...
procedure TOFI_FM_M_RQV.txtLocalExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
       btnGrava.Focused  then
...
procedure TOFI_FM_M_RQV.txtPesquisaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TOFI_FM_M_RQV.txtQtdeExit(Sender: TObject);
var
...
procedure TOFI_FM_M_RQV.txtTerceiroButtonClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_RQV.txtValorExit(Sender: TObject);
var
  SMPrincipal : TSMClient;
  seq_item: string;
begin
...
procedure TOFI_FM_M_RQV.txtVlrUnitarioExit(Sender: TObject);
var
  SMPrincipal : TSMClient;
  seq_item: string;
begin
...
procedure TOFI_FM_M_RQV.dbGridCalcCellColors(Sender: TObject; Field: TField;
  State: TGridDrawState; Highlight: Boolean; AFont: TFont; ABrush: TBrush);
begin
  inherited;
...
procedure TOFI_FM_M_RQV.dgDespesasIButtonClick(Sender: TObject);
begin
  inherited;
  try
...
procedure TOFI_FM_M_RQV.dgDespesasMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TOFI_FM_M_RQV.dgItensCalcCellColors(Sender: TObject; Field: TField;
  State: TGridDrawState; Highlight: Boolean; AFont: TFont; ABrush: TBrush);
begin
  inherited;
...
procedure TOFI_FM_M_RQV.dgItensDblClick(Sender: TObject);
begin
  inherited;
    ...
    begin
     if (dmGeral.OFI_CD_M_ORV_ITE.FieldByName('CANCELADA').AsBoolean = False) then
        begin
          ...
          if (pnlMecTer.Visible = false) then
             begin
               dmGeral.OFI_CD_M_ORV_ITE.Edit;
               txtQtde.SetFocus;
             end;
...
procedure TOFI_FM_M_RQV.dgItensIButtonClick(Sender: TObject);
begin
  inherited;
{ try
    dgItensIButton.Enabled := false;
...
procedure TOFI_FM_M_RQV.dgItensMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TOFI_FM_M_RQV.dpkDataExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
       btnGrava.Focused  then
...
procedure TOFI_FM_M_RQV.Atualizar_xBusca;
begin
  if xbusca_item = '' then
     begin
...
procedure TOFI_FM_M_RQV.btnAddDespesasClick(Sender: TObject);
begin
  inherited;
  pnlDespesas.Enabled  := True;
...
procedure TOFI_FM_M_RQV.btnAddItensClick(Sender: TObject);
begin
  inherited;
  pnlItens.Enabled  := True;
...
procedure TOFI_FM_M_RQV.btnAddItensKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
   if Key = vk_F2 then
...
procedure TOFI_FM_M_RQV.btnCancelarRegClick(Sender: TObject);
begin
  inherited;
  OFI_FM_M_RQV_CAN := TOFI_FM_M_RQV_CAN.Create(Self);
  OFI_FM_M_RQV_CAN.Show;
end;

procedure TOFI_FM_M_RQV.btnFiltroClick(Sender: TObject);
VAR
  xPesquisa:String;
begin
  inherited;
    ...
    if (cbbPesquisa.ItemIndex = 0) then
       begin
        ...
        dmGeral.OFI_CD_M_ORV.DataRequest(
                VarArrayOf([cbbPesquisa.ItemIndex,xPesquisa,'','2']));
       end
    else
       begin
         if (cbbPesquisa.ItemIndex = 5) then
          begin
            ...
            dmGeral.OFI_CD_M_ORV.Data :=
              dmGeral.OFI_CD_M_ORV.DataRequest(
                VarArrayOf([10,xPesquisa,dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').AsString,'2','1']));
          end
         else
          begin
            dmGeral.OFI_CD_M_ORV.Close;
            dmGeral.OFI_CD_M_ORV.Data :=
              dmGeral.OFI_CD_M_ORV.DataRequest(
                VarArrayOf([cbbPesquisa.ItemIndex,xPesquisa,dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').AsString,'2','1']));
         end;
       end;
...
procedure TOFI_FM_M_RQV.btnImprimirEspelhoClick(Sender: TObject);
var
...
begin
  ...
  if not dmGeral.OFI_CD_M_ORV.IsEmpty then
      begin
        ...
        OFI_FM_M_RQV_IMP := TOFI_FM_M_RQV_IMP.Create(Self);
        OFI_FM_M_RQV_IMP.Show;
      end
   else
      begin
...
procedure TOFI_FM_M_RQV.cbbMecanicoEnter(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_RQV.cbbMecanicoExit(Sender: TObject);
var
  seq_item:string;
begin
  inherited;
...
procedure TOFI_FM_M_RQV.cbbMecPaiEnter(Sender: TObject);
begin
  inherited;
  dmGeral.BUS_CD_C_FUN_MEC.Close;
  dmGeral.BUS_CD_C_FUN_MEC.Open;
...
procedure TOFI_FM_M_RQV.cbbMecPaiExit(Sender: TObject);
begin
  inherited;

  if cbbMecPai.Text='' then
    dmgeral.OFI_CD_M_ORV.FieldByname('ID_MECANICO').AsString := '';
end;

procedure TOFI_FM_M_RQV.cbbPesquisaChange(Sender: TObject);
begin
  inherited;
  txtPesquisa.Text := '';
  ...
  if cbbPesquisa.ItemIndex in [1,5]  then
    begin
      txtPesquisa.Visible := false;
      txtPesquisaData.Visible := true;
      txtPesquisaData.Top := 17;
      txtPesquisaData.Left := 110;
...
procedure TOFI_FM_M_RQV.cbbTerceiroEnter(Sender: TObject);
begin
  inherited;
  dmGeral.BusFornecedor(1,'%');
...
procedure TOFI_FM_M_RQV.cbbTerceiroExit(Sender: TObject);
var
  seq_item:string;
begin
  inherited;
...
procedure TOFI_FM_M_RQV.cbbTipoEnter(Sender: TObject);
begin
  inherited;
  dmGeral.BUS_PR_X_ORV_DES_TIP;
  cbbTipo.DropDown;
...
procedure TOFI_FM_M_RQV.cbbTipoExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused then
