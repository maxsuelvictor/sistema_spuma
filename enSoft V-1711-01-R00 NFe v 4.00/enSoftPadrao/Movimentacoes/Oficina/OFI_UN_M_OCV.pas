...
type
  TOFI_FM_M_OCV = class(TPAD_FM_X_PAD)
...
uses uDmGeral, PSQ_UN_X_CLI, PSQ_UN_X_ITE, PSQ_UN_X_VEI, OFI_RN_M_OCV,
  enConstantes, OFI_UN_M_OCV_CAN;


procedure TOFI_FM_M_OCV.acAdicionaExecute(Sender: TObject);
begin
  inherited;
  ...
  dmGeral.OFI_CD_M_OCV.FieldByName('tipo_orcamento').AsInteger := 1; // Veiculos (não tratores)
end;

procedure TOFI_FM_M_OCV.acAlterarExecute(Sender: TObject);
begin
  if Botoes(dso.DataSet, TAction(Sender).Tag,dmGeral.OFI_CD_M_OCV) then
    begin
        inherited;
         ...
         btnImprimirEspelho.Enabled := false;
         btnCancelamento.Enabled    := false;
         dmGeral.BusCliente(0,dmGeral.OFI_CD_M_OCV.FieldByName('ID_CLIENTE').Text);
         pnlItens.Enabled := False;
         dpkValidade.SetFocus;
    end;
end;

procedure TOFI_FM_M_OCV.acCancelarExecute(Sender: TObject);
begin
  if Botoes(dso.DataSet, TAction(Sender).Tag,dmGeral.OFI_CD_M_OCV) then
     begin
       inherited;
...
procedure TOFI_FM_M_OCV.acExcluirExecute(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_OCV.acGravarExecute(Sender: TObject);
var
  codigo: string;
begin

   if (OFI_RN_M_OCV.VerificarVlrTotais = false) then
     exit;
   ...
   if MessageDlg('Confirma a Gravação?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
     begin
       ...
       dmGeral.OFI_CD_M_OCV.DataRequest(VarArrayOf([0, codigo,'1']));
     end
   else
     begin
        dpkValidade.SetFocus;
...
procedure TOFI_FM_M_OCV.Atualizar_xBusca;
begin
  if xbusca_item = '' then
     begin
...
procedure  TOFI_FM_M_OCV.HabilitarDesabCampos(Ativar:Boolean);
begin
...
procedure TOFI_FM_M_OCV.AbrirBusca;
begin
...
procedure TOFI_FM_M_OCV.btnAddItensClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_OCV.btnFiltroClick(Sender: TObject);
var
  index:integer;
begin
   index := 0;
...
procedure TOFI_FM_M_OCV.btnImprimirEspelhoClick(Sender: TObject);
var
      ...
      begin
        ...
        // OFI_FR_M_OCV.Variables['ft_telfixo'] :=  QuotedStr('');
         ...
         if (not dmGeral.BUS_CD_C_CLI.IsEmpty) {and (not dmGeral.BUS_CD_C_CLI.FieldByName('TEL_FIXO').IsNull)} then
          ...
          begin
            OFI_FR_M_OCV.Variables['ft_telfixo']  :=  QuotedStr('');
            OFI_FR_M_OCV.Variables['ft_telmovel'] :=  QuotedStr('');
            OFI_FR_M_OCV.Variables['ft_email']    :=  QuotedStr('');
          end;
        dmGeral.BusCodigoRevListMestre(true,false,OFI_FM_M_OCV.Name,xCodLme,xRevLme,nil);
...
procedure TOFI_FM_M_OCV.Cancelaroramento1Click(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_OCV.Cancelaroramentosvencidos1Click(Sender: TObject);
var
  registros: String;
begin
  inherited;
...
procedure TOFI_FM_M_OCV.cbbAnalistaEnter(Sender: TObject);
begin
  inherited;
  dmGeral.BUS_CD_C_FUN.Close;
  dmGeral.BUS_CD_C_FUN.Data :=
...
procedure TOFI_FM_M_OCV.cbbAnalistaExit(Sender: TObject);
begin
  inherited;
  ...
  if (dpkValidade.Focused or txtIdCliente.Focused or txtNomeCliente.Focused or
      txtPlaca.Focused or txtKmAtual.Focused or cbbCondicaoPagamento.Focused) then
    begin
      exit;
    end;
...
procedure TOFI_FM_M_OCV.cbbCondicaoPagamentoEnter(Sender: TObject);
begin
  inherited;
  dmGeral.BusCondPgto(1,'%');
  CondPagAnt := dmGeral.OFI_CD_M_OCV.FieldBYName('ID_CONDICAO_PAG').AsString;
  cbbCondicaoPagamento.DropDown;
end;

procedure TOFI_FM_M_OCV.cbbCondicaoPagamentoExit(Sender: TObject);
begin
  inherited;
  ...
  if (dpkValidade.Focused or txtIdCliente.Focused or txtNomeCliente.Focused or txtPlaca.Focused or txtKmAtual.Focused) then
    begin
      exit;
    end;}
  ...
  if (dmGeral.OFI_CD_M_OCV_ITE.RecordCount > 0) and
     (CondPagAnt<>dmGeral.OFI_CD_M_OCV.FieldByName('ID_CONDICAO_PAG').AsString) then
     begin
        if MessageDlg('O valor unitário de cada item serão recalculados e os descontos serão zerados. Deseja continuar?', mtConfirmation, [mbYes, mbNo], 0) = mrNo then
         begin
           dmGeral.OFI_CD_M_OCV.FieldByName('ID_CONDICAO_PAG').AsString := CondPagAnt;
           dmGeral.BusCondPgto(0,dmGeral.OFI_CD_M_OCV.FieldByName('ID_CONDICAO_PAG').AsString);
           dmGeral.OFI_CD_M_OCV.FieldByName('INT_NOMECPG').AsString :=
               dmGeral.BUS_CD_C_CPG.FieldByName('DESCRICAO').AsString;
           exit;
         end;
...
procedure TOFI_FM_M_OCV.cbbPesquisaChange(Sender: TObject);
begin
  inherited;
  txtPesquisa.Text := '';
  if (cbbPesquisa.ItemIndex = 0) then
...
procedure TOFI_FM_M_OCV.dbGridCalcCellColors(Sender: TObject; Field: TField;
  State: TGridDrawState; Highlight: Boolean; AFont: TFont; ABrush: TBrush);
begin
  inherited;
...
procedure TOFI_FM_M_OCV.dgItensIButtonClick(Sender: TObject);
begin
  inherited;
  try
...
procedure TOFI_FM_M_OCV.dgItensMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TOFI_FM_M_OCV.dpkValidadeExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or btnGrava.Focused then
    begin
...
procedure TOFI_FM_M_OCV.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  dmGeral.OFI_CD_M_OCV.Close;
  FreeAndNil(OFI_FM_M_OCV);
end;

procedure TOFI_FM_M_OCV.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TOFI_FM_M_OCV.FormShow(Sender: TObject);
begin
  inherited;
  dmGeral.OFI_CD_M_OCV.Close;
...
procedure TOFI_FM_M_OCV.pnlItensExit(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_OCV.txtidClienteButtonClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_OCV.txtidClienteExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or
     btnGrava.Focused  then
...
procedure TOFI_FM_M_OCV.txtKmAtualExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or btnGrava.Focused then
    begin
   ...
   if (dpkValidade.Focused or txtIdCliente.Focused or txtNomeCliente.Focused or txtPlaca.Focused) then
    begin
      exit;
    end;
...
procedure TOFI_FM_M_OCV.txtNomeClienteExit(Sender: TObject);
begin
  inherited;
   if btnCancelar.Focused or btnGrava.Focused then
    begin
...
procedure TOFI_FM_M_OCV.txtBuscaItemButtonClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_OCV.txtBuscaItemExit(Sender: TObject);
var
...
procedure TOFI_FM_M_OCV.txtBuscaItemKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
  if key = vk_f7 then
...
procedure TOFI_FM_M_OCV.txtCnpjCpfExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or btnGrava.Focused then
    begin
...
procedure TOFI_FM_M_OCV.txtDescEnter(Sender: TObject);
begin
  inherited;
   xPercDescPerm := dmGeral.BuscarDescItem(dmGeral.OFI_CD_M_OCV_ITE);
end;

procedure TOFI_FM_M_OCV.txtDescServicoEnter(Sender: TObject);
begin
  inherited;
  dmgeral.OFI_CD_M_OCVdes_servicos.OnChange:= dmgeral.OFI_CD_M_OCVdes_servicosChange;
end;

procedure TOFI_FM_M_OCV.txtDescServicoExit(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_M_OCV.txtPesquisaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TOFI_FM_M_OCV.txtPlacaButtonClick(Sender: TObject);
begin
  inherited;
  ...
  if not PSQ_FM_X_VEI.BUS_CD_C_VEI.IsEmpty then
     begin
        dmGeral.OFI_CD_M_OCV.FieldByName('placa').Text :=
...
procedure TOFI_FM_M_OCV.txtPlacaExit(Sender: TObject);
begin
  inherited;
  if btnCancelar.Focused or btnGrava.Focused then
    begin
  ...
  if (trim(dmGeral.OFI_CD_M_OCV.FieldByName('PLACA').AsString) = '') then
    begin
  ...
  dmGeral.OFI_CD_M_OCV.FieldByName('PLACA').Text := UpperCase(txtPlaca.text);

  dmGeral.OFI_CD_M_OCV.FieldByName('INT_NOMEVEI').Text := '';

  dmGeral.BUS_CD_C_VEI.Close;
  dmGeral.BUS_CD_C_VEI.Data :=
  dmGeral.BUS_CD_C_VEI.DataRequest(VarArrayOf([3, trim(dmGeral.OFI_CD_M_OCV.FieldByName('PLACA').Text)]));
  ...
  if (trim(dmGeral.BUS_CD_C_VEI.FieldByName('tipo').AsString) = '0') then
    begin
      dmGeral.OFI_CD_M_OCV.FieldByName('PLACA').AsString :=
                               dmGeral.BUS_CD_C_VEI.FieldByName('ID_PLACA').AsString;
      dmGeral.OFI_CD_M_OCV.FieldByName('INT_NOMEVEI').Text :=
                  dmGeral.BUS_CD_C_VEI.FieldByName('DESCRICAO').AsString;
    end
  else
    begin
      ShowMessage('Deve ser do Tipo veiculo.');
      txtPlaca.SetFocus;
      Exit;
    end; }
end;

procedure TOFI_FM_M_OCV.txtQtdeEnter(Sender: TObject);
var
      ...
      if dmGeral.OFI_CD_M_OCV_ITE.State in [dsInsert] then
         begin
          ...
          // id_cor := dmGeral.OFI_CD_M_OCV_ITE.FieldByName('ID_COR').Text;
           ...
           if dmGeral.LocalizarRegItem(dmGeral.OFI_CD_M_OCV_ITE,OFI_FM_M_OCV.Name,id_item,'0','0','','') then
              begin
...
procedure TOFI_FM_M_OCV.txtQtdeExit(Sender: TObject);
var
...
procedure TOFI_FM_M_OCV.txtVlrDescEnter(Sender: TObject);
begin
  inherited;
   xPercDescPerm := dmGeral.BuscarDescItem(dmGeral.OFI_CD_M_OCV_ITE);
end;
...
procedure TOFI_FM_M_OCV.txtVlrUnitarioExit(Sender: TObject);
var
  SMPrincipal : TSMClient;
  seq_item: string;
begin
