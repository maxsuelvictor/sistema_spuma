...
begin
  ...
  PCP_RN_M_ROM.PcpVerifQtdeRomIcf;
       ...
       begin
         if (cbbPesquisa.ItemIndex = 6)  then
             begin
               ...
               dmGeral.PCP_CD_M_ROM.Data :=
               dmGeral.PCP_CD_M_ROM.DataRequest(
                      VarArrayOf([cbbPesquisa.ItemIndex+5,dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').Text, cbAno.Text, txtPesquisa.Text]));
             end
         else
             begin
               if (cbbPesquisa.ItemIndex = 7)  then
                   begin
                     ...
                     dmGeral.PCP_CD_M_ROM.Data :=
                     dmGeral.PCP_CD_M_ROM.DataRequest(
                            VarArrayOf([cbbPesquisa.ItemIndex+1,dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').Text, txtPesquisa.Text]));
                   end
               else
                   begin
                      dmGeral.PCP_CD_M_ROM.Close;
...
begin
  ...
  if not dmGeral.PCP_CD_M_ROM.IsEmpty then
    begin
      ...
      IdRom :=  dmGeral.PCP_CD_M_ROM.FieldByName('id_rom').Text;

      try

      dmGeral.PCP_CD_M_ROM.Filtered := true;
      dmGeral.PCP_CD_M_ROM.Filter := 'ID_ROM = ' + IdRom;
      ...
      PCP_FR_M_ROM.ShowReport();
         ...
         dmGeral.PCP_CD_M_ROM.Filtered := false;
      end;
...
begin
  ...
  cbAno.Visible := false;
  txtPesquisa.Width := 143;
  txtPesquisa.left  := 140;

  ano := FormatDateTime('yyyy',xDataSis);
  if cbbPesquisa.ItemIndex = 6 then
     begin
       for i := 0 to cbAno.Items.Count - 1 do
           begin
             if cbAno.Items[i] = ano then
                begin
                   cbAno.ItemIndex := i;
                end;
           end;

       cbAno.text := FormatDateTime('yyyy',xDataSis);
       cbAno.Visible := true;
       txtPesquisa.Width := 87;
       txtPesquisa.left  := 192;
     end;
  txtPesquisa.setfocus;
end;

procedure TPCP_FM_M_ROM.btnVlrCargaClick(Sender: TObject);
var
  PathImg: String;
...
begin
  ...
  Application.CreateForm(TPCP_FM_M_ROM_VLC,PCP_FM_M_ROM_VLC);
  PCP_FM_M_ROM_VLC.ShowModal;
  FreeAndNil(PCP_FM_M_ROM_VLC);
...
begin
  ...
  dmGeral.BUS_CD_C_FU2.Data :=
       dmGeral.BUS_CD_C_FU2.DataRequest(VarArrayOf([0, xFuncionario]));

  if dmGeral.BUS_CD_C_FU2.FieldByName('sgq_vis_vlr_tot_carga').AsBoolean then
     begin
       btnVlrCarga.Visible := true;
     end;

  txtPesquisa.SetFocus;

  cbAno.text := FormatDateTime('yyyy',xDataSis);
  txtAnoCorrentePcpRom.Text := dmGeral.CAD_CD_C_PAR_CTR.FieldByName('pcp_rom_ano_corrente').AsString;
    ...
    begin
      ...
      if dmgeral.PCP_CD_M_ROM_ITE.Locate('id_item;id_cor;id_tamanho;nome_ite',
         ...
         begin
            ...
            dmgeral.PCP_CD_M_ROM_ITE.FieldByName('int_undvenda').AsString := dmgeral.BUS_CD_M_PED_ITE.FieldByName('int_id_und_venda').AsString;
            dmgeral.PCP_CD_M_ROM_ITE.FieldByName('ID_COR').AsInteger      := dmgeral.BUS_CD_M_PED_ITE.FieldByName('ID_COR').AsInteger;
            dmgeral.PCP_CD_M_ROM_ITE.FieldByName('int_nomecor').AsString  := dmgeral.BUS_CD_M_PED_ITE.FieldByName('int_nomecor').AsString;
            ...
            dmgeral.PCP_CD_M_ROM_ITE.FieldByName('QTDE_CONFERIDA').AsCurrency  := 0;
         end;
  ...
  if (btnCancelar.Enabled) and
     (btnGrava.Enabled) then
      begin
        ...
        Application.CreateForm(TPCP_FM_M_ROM_ICF,PCP_FM_M_ROM_ICF);

        PCP_FM_M_ROM_ICF.lblTitulo.caption := 'Conferência da Carga: ' + dmGeral.PCP_CD_M_ROMid_rom.Text;
        PCP_FM_M_ROM_ICF.RetirarItem := false;
        PCP_FM_M_ROM_ICF.ShowModal;
...
begin
  ...
  try
    ...
    if not (dmGeral.PCP_CD_M_ROM_PED_ICF.IsEmpty) then
       begin
         ShowMessage('Não é permitido retirar o pedido, pois já existe itens conferido para este pedido.');
         abort;
       end;
...
begin
  inherited;
  if dmGeral.PCP_CD_M_ROM.state in [dsInsert,dsEdit] then
     begin
       SomarQtdePedido;
     end;
end;
...
procedure TPCP_FM_M_ROM.RetirarRomIte;
begin
     ...
     begin
       if dmgeral.PCP_CD_M_ROM_ITE.Locate('id_item;id_cor;id_tamanho;nome_ite',
          ...
          begin
            ...
            if (dmgeral.PCP_CD_M_ROM_ITE.FieldByName('qtde').AsCurrency = 0) and
               (dmgeral.PCP_CD_M_ROM_ITE.FieldByName('qtde_conferida').AsCurrency = 0) then
                dmgeral.PCP_CD_M_ROM_ITE.delete;
...
procedure TPCP_FM_M_ROM.SpeedButton1Click(Sender: TObject);
begin
  inherited;
  if (btnCancelar.Enabled) and
     (btnGrava.Enabled) then
      begin
        ...
        PCP_FM_M_ROM_ICF.RetirarItem := true;
        PCP_FM_M_ROM_ICF.lblTitulo.Color := clBlue;
        PCP_FM_M_ROM_ICF.btnGravar.Visible  := false;
        PCP_FM_M_ROM_ICF.lblTitulo.caption  := 'Retirar item da carga';
        PCP_FM_M_ROM_ICF.txtPedido.TabStop  := true;
        PCP_FM_M_ROM_ICF.txtPedido.color    := clWhite;
        PCP_FM_M_ROM_ICF.txtPedido.enabled  := true;
        PCP_FM_M_ROM_ICF.txtPedido.ReadOnly := false;
        PCP_FM_M_ROM_ICF.btnRealocarEtiq.Visible := false;

        // PCP_FM_M_ROM_ICF.visible := false;
        // PCP_FM_M_ROM_ICF.FormStyle := fsNormal;
        PCP_FM_M_ROM_ICF.ShowModal;
        FreeAndNil(PCP_FM_M_ROM_ICF);



        //PCP_FM_M_ROM_ICF.btnAddIteClick(PCP_FM_M_ROM_ICF);
      end;
end;
      ...
      begin
        ...
        dmGeral.BUS_CD_M_PCA.Close;
           dmGeral.BUS_CD_M_PCA.Data :=
              dmGeral.BUS_CD_M_PCA.DataRequest(VarArrayOf([4, txtCodCarreg.Text,dmGeral.CAD_CD_C_PAR_CTR.FieldByName('pcp_rom_ano_corrente').AsInteger]));
        if dmGeral.BUS_CD_M_PCA.IsEmpty then
           begin
             ShowMessage('Carregamento não existe.');
             dmgeral.PCP_CD_M_ROM.FieldByName('id_pca').AsString := '';
             dmgeral.PCP_CD_M_ROM.FieldByName('seq_pca_carreg_anual').AsString := CodAntCarregamento;
             dmGeral.BUS_CD_M_PCA.Close;
             txtCodCarreg.SetFocus;
             exit;
           end;

        dmgeral.PCP_CD_M_ROM.FieldByName('id_pca').AsInteger :=
            dmgeral.BUS_CD_M_PCA.FieldByName('id_carregamento').AsInteger;
            ...
            begin
              if (dmgeral.BUS_CD_M_PCA_PED.FieldByName('int_situacaoped').AsInteger = 2) and  // em produção
                 ...
                 begin
                    ...
                    if IdRom <> 0 then
                       begin
                         ShowMessage('O pedido: ' + dmgeral.BUS_CD_M_PCA_PED.FieldByName('id_pedido').AsString + ', já existe no relatório de carga Nº '+IntToStr(IdRom)+'.');
                         txtCodCarreg.Text := '';
                         txtCodCarreg.setfocus;
        ...
        if (dmgeral.PCP_CD_M_ROM_PED.RecordCount > 0) then
            begin
              ...
              dmgeral.PCP_CD_M_ROM.FieldByName('seq_pca_carreg_anual').AsInteger := dmGeral.BUS_CD_M_PCA_PED.FieldByName('int_seq_carreg_anual').AsInteger;
              PCP_RN_M_ROM.PcpCalculaQtdeTotalPedRom;
            end;
...
begin
  ...
  if dmgeral.PCP_CD_M_ROM_PED.FieldByName('ID_PEDIDO').Text = '' then
    ...
    begin
      ...
      if dmgeral.BUS_CD_M_PED.FieldByName('pcp_id_opr').AsInteger = 0 then
         begin
           ShowMessage('O pedido de venda precisa estar vinculado em uma ordem de produção.');
           dmgeral.PCP_CD_M_ROM_PED.FieldByName('ID_PEDIDO').Text := '';
           txtPedido.SetFocus;
           abort;
         end;
