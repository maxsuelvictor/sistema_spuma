...
type
  TOFI_FM_R_OFP_POS = class(TPAD_FM_X_REL)
    lblPeriodo: TLabel;
    ...
    Label1: TLabel;
    OFI_CD_R_OFV_POS_001tipo_ordem: TIntegerField;
    OFI_CD_R_OFV_POS_001chassi: TWideStringField;
    OFI_CD_R_OFV_POS_002tipo_ordem: TIntegerField;
    OFI_CD_R_OFV_POS_002chassi: TWideStringField;
    OFI_CD_R_OFV_POS_003tipo_ordem: TIntegerField;
    OFI_CD_R_OFV_POS_003chassi: TWideStringField;
    OFI_CD_R_OFV_POS_EST: TClientDataSet;
    OFI_CD_R_OFV_POS_ESTid_empresa: TIntegerField;
    OFI_CD_R_OFV_POS_ESTemp_fantasia: TWideStringField;
    OFI_CD_R_OFV_POS_ESTqtde_nota: TFMTBCDField;
    OFI_CD_R_OFV_POS_ESTqtde_veiculo: TFMTBCDField;
    OFI_CD_R_OFV_POS_ESTvlr_bruto: TFloatField;
    OFI_CD_R_OFV_POS_ESTvlr_desconto: TFMTBCDField;
    OFI_CD_R_OFV_POS_ESTvlr_despesa: TFMTBCDField;
    OFI_CD_R_OFV_POS_ESTvlr_liquido: TFloatField;
    OFI_CD_R_OFV_POS_ESTvlr_liquido_ser: TFloatField;
    OFI_CD_R_OFV_POS_ESTvlr_liquido_pro: TFloatField;
    OFI_FR_R_OFV_POS_EST: TfrxReport;
    OFI_DB_R_OFV_POS_EST: TfrxDBDataset;
    OFI_CD_R_OFV_POS_001id_meq: TIntegerField;
    OFI_CD_R_OFV_POS_001int_nomemeq: TWideStringField;
    OFI_CD_R_OFV_POS_001int_seriemeq: TWideStringField;
    OFI_CD_R_OFV_POS_001int_modelomeq: TWideStringField;
    OFI_CD_R_OFV_POS_002id_meq: TIntegerField;
    OFI_CD_R_OFV_POS_002int_nomemeq: TWideStringField;
    OFI_CD_R_OFV_POS_002int_seriemeq: TWideStringField;
    OFI_CD_R_OFV_POS_002int_modelomeq: TWideStringField;
    tsMaqEquip: TTabSheet;
    lboxMaqEquip: TListBox;
    Label2: TLabel;
    pmExcluirMaqEquip: TPopupMenu;
    btnExcluirMaq: TMenuItem;
    OFI_CD_R_OFV_POS_001int_placa_meq: TWideStringField;
    OFI_CD_R_OFV_POS_001int_nomecom_meq: TWideMemoField;
    OFI_CD_R_OFV_POS_002int_placa_meq: TWideStringField;
    ...
    procedure lboxMaqEquipKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure btnExcluirMaqClick(Sender: TObject);
  private
    ...
    var
      xCondEmpresa,xCondFuncionario,xCondMaquina,xCondPag,xCondAnalista,xCondTipoOrdem,xOpcoes:string;
  public
...
var
  OFI_FM_R_OFP_POS: TOFI_FM_R_OFP_POS;

implementation

{$R *.dfm}

uses uDmGeral, enFunc, PSQ_UN_X_FUN, enConstantes, PSQ_UN_X_VEI, PSQ_UN_X_MEQ;

function TOFI_FM_R_OFP_POS.TestarCampos;
var
  mens: String;
begin
  result := True;
...
procedure TOFI_FM_R_OFP_POS.PreencherCondicao;
var
  i: integer;
  opcoesEmpresa,opcoesFuncionario,opcoesMaquina,
...
begin
  ...
  if lboxFuncionario.Count > 0 then
   ...
   // Máquina --------------------------------------------------------------
  xCondMaquina := '';
  if lboxMaqEquip.Count > 0 then
     begin
       for i := 0 to lboxMaqEquip.Count - 1 do
          begin
            if xCondMaquina <> '' then
               begin
                 xCondMaquina  :=
                 xCondMaquina + ',' +''''+trim(copy(lboxMaqEquip.Items[i],1,9))+'''';
                 opcoesMaquina :=
                 opcoesMaquina +'                    '+lboxMaqEquip.Items[i] + #13+#10;
               end;
            if xCondMaquina = '' then
               begin
                 xCondMaquina  :=
  ...
  begin
   ...
   xOpcoes := xOpcoes + 'Máquina/Equip.....: ' + opcoesMaquina+#13+#10;
...
procedure TOFI_FM_R_OFP_POS.ExibirRel;
var
   ...
   if (lbxRelatorios.Items[lbxRelatorios.ItemIndex] = '01 - Detalhado') then
        ...
        OFI_CD_R_OFV_POS_001.Data :=
                OFI_CD_R_OFV_POS_001.DataRequest(
                      VarArrayOf([rDtaIni,rDtaFin,rOrdIni,rOrdFin,rSituacao,xCondEmpresa,xCondFuncionario,
                                  '',xCondPag,xCondAnalista,xCondTipoOrdem,cbTipoPeriodo.ItemIndex,'4','',xCondMaquina]));
      ...
      begin
        if ((lbxRelatorios.Items[lbxRelatorios.ItemIndex] = '05 - Por Mecânico')) or
          ...
          begin
            if (lbxRelatorios.Items[lbxRelatorios.ItemIndex] = '10 - Estatística') then
              begin
                OFI_CD_R_OFV_POS_EST.Active:= False;
                OFI_CD_R_OFV_POS_EST.Close;
                OFI_CD_R_OFV_POS_EST.Data :=
                      OFI_CD_R_OFV_POS_EST.DataRequest(
                            VarArrayOf([rDtaIni,rDtaFin,rOrdIni,rOrdFin,rSituacao,xCondEmpresa,xCondFuncionario,
                                        '',xCondPag,xCondAnalista,xCondTipoOrdem,cbTipoPeriodo.ItemIndex,'4','',xCondMaquina]));
              end
            else
              begin
                ...
                OFI_CD_R_OFV_POS_002.Data :=
                          OFI_CD_R_OFV_POS_002.DataRequest(
                                VarArrayOf([rDtaIni,rDtaFin,rOrdIni,rOrdFin,rSituacao,xCondEmpresa,xCondFuncionario,
                                            '',xCondPag,xCondAnalista,xCondTipoOrdem,lbxRelatorios.ItemIndex,cbTipoPeriodo.ItemIndex,'4','',xCondMaquina]));
              end;

            if (not OFI_CD_R_OFV_POS_002.IsEmpty) or (not OFI_CD_R_OFV_POS_EST.IsEmpty) then
              begin
                PathImg := ExtractFilePath(Application.ExeName)+'emp'+dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').Text+'\LogoEmpresa.jpg';
                   ...
                   if (lbxRelatorios.Items[lbxRelatorios.ItemIndex] = '03 - Por Máquina/Equip./Veículo') then
                     begin
                       OFI_CD_R_OFV_POS_002.IndexFieldNames := 'ID_MEQ;DTA_EMISSAO';
                       if FileExists(PathImg) then
                           begin
                   ...
                   if (lbxRelatorios.Items[lbxRelatorios.ItemIndex] = '06 - Máquina/Equip./Veículo com entrega atrasada') then
                     begin
                  ...
                  if (lbxRelatorios.Items[lbxRelatorios.ItemIndex] = '10 - Estatística') then
                     begin
                       OFI_CD_R_OFV_POS_EST.IndexFieldNames := 'ID_EMPRESA';
                       if FileExists(PathImg) then
                           begin
                             LogoEmpresa := TfrxPictureView(OFI_FR_R_OFV_POS_EST.FindObject('imgEmpresa3'));
                             if Assigned(LogoEmpresa) then
                               LogoEmpresa.Picture.LoadFromFile(PathImg);
                           end;
                         dmGeral.BusCodigoRevListMestre(true,false,OFI_FR_R_OFV_POS_EST.Name,xCodLme,xRevLme,nil);
                         OFI_FR_R_OFV_POS_EST.Variables['ft_codlme'] := QuotedStr(dmGeral.MontarCodRevLme(xCodLme,xRevLme));
                         OFI_FR_R_OFV_POS_EST.Variables['Assin_enorth'] := QuotedStr(CAssin_Enorth);
                         OFI_FR_R_OFV_POS_EST.Variables['Opcoes'] := QuotedStr(xOpcoes);
                         OFI_FR_R_OFV_POS_EST.PrepareReport();
                         OFI_FR_R_OFV_POS_EST.ShowReport();
                     end;
              end
            else
              begin
...
procedure TOFI_FM_R_OFP_POS.btnExcluirMaqClick(Sender: TObject);
var
  i:integer;
begin
  inherited;
    if lboxMaqEquip.ItemIndex = -1 then
     begin
       showmessage('Nenhum registro foi selecionado.');
       exit;
     end;

  lboxMaqEquip.DeleteSelected;
end;

procedure TOFI_FM_R_OFP_POS.btnImprimirClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.btnRemoverAnalistaClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.btnRemoverRegistroFuncionarioClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.btnRemoverTipoOrdServClick(Sender: TObject);
var
  i:integer;
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.btnRemoverTodasCondPagClick(Sender: TObject);
var
  i:integer;
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.btnRemoverTodosEmpresaClick(Sender: TObject);
var
  i:integer;
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.btnRemoverVeiculoClick(Sender: TObject);
var
  i:integer;
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.cbbFiltroTipoChange(Sender: TObject);
begin
  inherited;
  if (cbbFiltroTipo.ItemIndex = 0) then
    begin
...
procedure TOFI_FM_R_OFP_POS.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  FreeAndNil(OFI_FM_R_OFP_POS);
end;

procedure TOFI_FM_R_OFP_POS.FormShow(Sender: TObject);
begin
  inherited;
  if (dmgeral.CAD_CD_C_PAR_MOD.FieldByName('OMP').AsBoolean=true) then
    begin
      tsMaqEquip.TabVisible := true;
      tsVeiculo.TabVisible := false;
    end
  else
    begin
      tsMaqEquip.TabVisible := false;
...
procedure TOFI_FM_R_OFP_POS.lboxAnalistaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
   if key = vk_f3 then
...
procedure TOFI_FM_R_OFP_POS.lboxCondDisponivelDblClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.lboxCondSelecionadoDblClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.lboxEmpresasDblClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.lboxEmpresaSelecionadaDblClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.lboxFuncionarioKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
  if key = vk_f3 then
...
procedure TOFI_FM_R_OFP_POS.lboxMaqEquipKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
   if key = vk_f3 then
     begin
        ...
        if not PSQ_FM_X_MEQ.BUS_CD_C_MEQ.IsEmpty then
           begin
             if not dmGeral.ExisteRegNoListBox(lboxVeiculoDisponivel,9,StrZero(PSQ_FM_X_MEQ.BUS_CD_C_MEQ.FieldByName('ID_MEQ').AsFloat,9,0)) then
                begin
                  lboxMaqEquip.Items.Add(StrZero(PSQ_FM_X_MEQ.BUS_CD_C_MEQ.FieldByName('ID_MEQ').AsFloat,9,0) +
...
procedure TOFI_FM_R_OFP_POS.lboxTipoOrdServDisponivelDblClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.lboxTipoOrdServSelecionadoClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.lboxVeiculoDisponivelKeyDown(Sender: TObject;
...
procedure TOFI_FM_R_OFP_POS.lbxRelatoriosClick(Sender: TObject);
begin
  inherited;

  cbbFiltroTipo.Enabled   := true;
  cbTipoPeriodo.Enabled   := true;

  cbTipoPeriodo.Items.Clear;
  cbTipoPeriodo.Items.Add('Emissão');
  cbTipoPeriodo.Items.Add('Fechamento');

  cbTipoPeriodo.ItemIndex := 0;
 ...
 if (lbxRelatorios.ItemIndex = 7) then
  ...
  if lbxRelatorios.ItemIndex = 9 then
     begin
       cbbFiltroTipo.Enabled   := false;
       cbbFiltroTipo.ItemIndex := 0;


       cbTipoPeriodo.Items.Clear;
       cbTipoPeriodo.Items.Add('Emissão NF');
       cbTipoPeriodo.Items.Add('Fechamento');
       cbTipoPeriodo.ItemIndex := 0;
       cbTipoPeriodo.Enabled := false;
       cbbSituacao.ItemIndex := 3;
       cbbSituacao.Enabled   := false;
     end;
end;

procedure TOFI_FM_R_OFP_POS.MenuItemCondPagClick(Sender: TObject);
var
  i: integer;
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.menuItemEmpresasClick(Sender: TObject);
var
  i: integer;
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.MenuItemTipoOrdServClick(Sender: TObject);
var
  i: integer;
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.MenuItemVeiculoClick(Sender: TObject);
var
  i: integer;
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.OFI_FR_R_OFV_POS_05BeforePrint(
...
procedure TOFI_FM_R_OFP_POS.OFI_FR_R_OFV_POS_08BeforePrint(
  Sender: TfrxReportComponent);
begin
  inherited;
...
procedure TOFI_FM_R_OFP_POS.OFI_FR_R_OFV_POS_09BeforePrint(
  Sender: TfrxReportComponent);
begin
  inherited;
   if (OFI_CD_R_OFV_POS_003.FieldByName('INT_NOMEMEC').AsString <>'') then
...
procedure TOFI_FM_R_OFP_POS.OFI_FR_R_OFV_POS_10BeforePrint(
  Sender: TfrxReportComponent);
begin
  inherited;
   {if (OFI_CD_R_OFV_POS_003.FieldByName('INT_NOMEMEC').AsString <>'') then
