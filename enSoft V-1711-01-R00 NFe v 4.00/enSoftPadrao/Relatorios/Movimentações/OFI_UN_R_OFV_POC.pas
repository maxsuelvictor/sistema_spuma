...
type
  TOFI_FM_R_OFV_POC = class(TPAD_FM_X_REL)
    Label1: TLabel;
    Label2: TLabel;
...
var
  OFI_FM_R_OFV_POC: TOFI_FM_R_OFV_POC;
...
procedure TOFI_FM_R_OFV_POC.btnImprimirClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_OFV_POC.btnRemoverRegistroFuncionarioClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_OFV_POC.btnRemoverTodosEmpresaClick(Sender: TObject);
var
  i:integer;
begin
  inherited;
...
procedure TOFI_FM_R_OFV_POC.ExibirRel;
var
      ...
      begin
        ...
        OFI_CD_R_OFV_POC_01.Data :=
                OFI_CD_R_OFV_POC_01.DataRequest(
                      VarArrayOf([rDtaIni,rDtaFin,xCondFuncionario,rSituacao,xCondEmpresa,'1']));
    ...
    begin
      ...
      if not OFI_CD_R_OFV_POC_02.IsEmpty then
         begin
           ...
           if (lbxRelatorios.Items[lbxRelatorios.ItemIndex] = '05 - Por Veículo') then
             begin
...
procedure TOFI_FM_R_OFV_POC.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  inherited;
  FreeAndNil(OFI_FM_R_OFV_POC);
end;

procedure TOFI_FM_R_OFV_POC.FormShow(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_OFV_POC.lboxEmpresasDblClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_OFV_POC.lboxEmpresaSelecionadaDblClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_OFV_POC.lboxFuncionarioKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
  if key = vk_f3 then
...
procedure TOFI_FM_R_OFV_POC.menuItemEmpresasClick(Sender: TObject);
var
  i: integer;
begin
  inherited;
...
procedure TOFI_FM_R_OFV_POC.PreencherCondicao;
var
  i: integer;
...
function TOFI_FM_R_OFV_POC.TestarCampos: Boolean;
var
  mens: String;
begin
  result := True;
