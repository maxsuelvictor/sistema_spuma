...
type
  TOFI_FM_R_HIP = class(TPAD_FM_X_REL)
    Label1: TLabel;
    Label2: TLabel;
    ...
    OFI_CD_R_HIVid_meq: TIntegerField;
    OFI_CD_R_HIVint_nomemeq: TWideStringField;
    OFI_CD_R_HIVint_nomemde: TWideStringField;
    OFI_CD_R_HIVint_nomecom_meq: TWideMemoField;
    OFI_CD_R_HIVint_chassi_meq: TWideStringField;
    OFI_CD_R_HIVint_placa_meq: TWideStringField;
...
var
  OFI_FM_R_HIP: TOFI_FM_R_HIP;

implementation

{$R *.dfm}

uses uDmGeral, enFunc, PSQ_UN_X_VEI, enConstantes, PSQ_UN_X_MEQ;

procedure TOFI_FM_R_HIP.btnImprimirClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_HIP.btnRemoverTodosEmpresaClick(Sender: TObject);
var
  i:integer;
begin
  inherited;
...
procedure TOFI_FM_R_HIP.btnRemoverVeiculoClick(Sender: TObject);
var
  i:integer;
begin
  inherited;
...
procedure TOFI_FM_R_HIP.ExibirRel;
var
...
begin
  ...
  OFI_CD_R_HIV.Close;
  OFI_CD_R_HIV.Data :=
  OFI_CD_R_HIV.DataRequest(VarArrayOf([lbxRelatorios.ItemIndex, rDtaIni,rDtaFin,xCondEmpresa,'','4',xCondVeiculo]));
...
procedure TOFI_FM_R_HIP.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  FreeAndNil(OFI_FM_R_HIP);
end;

procedure TOFI_FM_R_HIP.FormShow(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_HIP.lboxEmpresasDblClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_HIP.lboxEmpresaSelecionadaDblClick(Sender: TObject);
begin
  inherited;
...
procedure TOFI_FM_R_HIP.lboxVeiculoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
  if key = vk_f3 then
     begin
        ...
        if not PSQ_FM_X_MEQ.BUS_CD_C_MEQ.IsEmpty then
           begin
             if not dmGeral.ExisteRegNoListBox(lboxVeiculo,9,StrZero(PSQ_FM_X_MEQ.BUS_CD_C_MEQ.FieldByName('ID_MEQ').AsFloat,9,0)) then
                begin
                  lboxVeiculo.Items.Add(StrZero(PSQ_FM_X_MEQ.BUS_CD_C_MEQ.FieldByName('ID_MEQ').AsFloat,9,0) +
...
procedure TOFI_FM_R_HIP.menuItemEmpresasClick(Sender: TObject);
var
  i: integer;
begin
  inherited;
...
procedure TOFI_FM_R_HIP.PreencherCondicao;
var
  i: integer;
...
begin
  ...
  if lboxVeiculo.Count > 0 then
   ...
   xOpcoes := xOpcoes + 'Máq/Equip/Veículo.: ' + opcoesVeiculo+#13+#10;

end;

function TOFI_FM_R_HIP.TestarCampos: Boolean;
var
  mens: String;
begin
  result := True;
