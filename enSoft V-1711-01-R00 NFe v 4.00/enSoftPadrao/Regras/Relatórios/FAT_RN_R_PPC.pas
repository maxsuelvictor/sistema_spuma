...
var
      ...
      xCondVendedor, xCondPisCofins, xCondFornecedor: string;
procedure ExibirRel(lbxRelatorios,lbxEmpresas,lbxEstoque,lbxCfop,lbxPagamento,lbxVendedor,lbxPisCofins,lbxFornecedor:TListBox;
                    deInicial,deFinal:TJvDateEdit; Fastreport1,Fastreport2:TfrxReport);
procedure PreencherCondicao(lbxEmpresas,lbxEstoque,lbxCfop,lbxPagamento,lbxVendedor,lbxPisCofins,lbxFornecedor:TListBox;
...
begin
   ...
   PreencherCondicao(lbxEmpresas,lbxEstoque,lbxCfop,lbxPagamento,lbxVendedor,lbxPisCofins,lbxFornecedor,deInicial,deFinal);
   ...
   if not dmGeral.CAD_CD_R_ITE.IsEmpty then
      begin
        ...
        if lbxRelatorios.Items[lbxRelatorios.ItemIndex] = 'Vendas por Forma de Pagamento' then
          begin
        ...
        if lbxRelatorios.Items[lbxRelatorios.ItemIndex] = 'Vendas por Vendedor' then
          begin
            if FileExists(PathImg) then
               begin
       ...
       for i := 0 to lbxPisCofins.Count - 1 do
          begin
            if xCondPisCofins <> '' then
               begin
                 xCondPisCofins :=
                 xCondPisCofins + ',' +''''+trim(copy(lbxPisCofins.Items[i],1,9))+'''';
               end;
            if xCondPisCofins = '' then
               begin
                 xCondPisCofins :=
