...
implementation
Uses uDmGeral,enConstantes, uProxy, CAD_UN_R_GRU;
procedure ExibirRel(lbxRelatorios:TListBox;Fastreport1:TfrxReport);
var
  PathImg: String;
...
begin
  if lbxRelatorios.ItemIndex = -1 then
   ...
   CAD_FM_R_GRU.CAD_CD_R_GRU.Close;
   CAD_FM_R_GRU.CAD_CD_R_GRU.Data :=
   CAD_FM_R_GRU.CAD_CD_R_GRU.DataRequest(
           VarArrayOf([1,'%']));

   if not CAD_FM_R_GRU.CAD_CD_R_GRU.IsEmpty then
      begin
        PathImg := ExtractFilePath(Application.ExeName)+'emp'+dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').Text+'\LogoEmpresa.jpg';

        if lbxRelatorios.Items[lbxRelatorios.ItemIndex] = 'Listagem de Grupo de Produtos' then
          begin
