...
implementation
Uses uDmGeral,enConstantes, uProxy, CAD_UN_R_CNE;
procedure ExibirRel(lbxRelatorios:TListBox;Fastreport1:TfrxReport);
var
  PathImg: String;
...
begin
   ...
   CAD_FM_R_CNE.CAD_CD_R_CNE.Close;
   CAD_FM_R_CNE.CAD_CD_R_CNE.Open;

   if not CAD_FM_R_CNE.CAD_CD_R_CNE.IsEmpty then
      begin
        PathImg := ExtractFilePath(Application.ExeName)+'emp'+dmGeral.CAD_CD_C_PAR.FieldByName('ID_EMPRESA').Text+'\LogoEmpresa.jpg';

        if lbxRelatorios.Items[lbxRelatorios.ItemIndex] = 'Listagem de CNAE - Classificação Nacional de Atividades Econômicas' then
          begin
